---
site: blogdown:::blogdown_site
---

# Analysis of movies based on IMDB dataset

```{r, setup, echo=FALSE}
knitr::opts_chunk$set(
  message = FALSE, 
  warning = FALSE, 
  tidy=FALSE,     # display code as typed
  size="small")   # slightly smaller font for code
options(digits = 3)

# default figure size
knitr::opts_chunk$set(
  fig.width=6.75, 
  fig.height=6.75,
  fig.align = "center"
)
```

```{r load-libraries, warning=FALSE, message=FALSE, echo=FALSE}
library(tidyverse)  # Load ggplot2, dplyr, and all the other tidyverse packages
library(mosaic)
library(ggthemes)
library(lubridate)
library(fivethirtyeight)
library(here)
library(skimr)
library(janitor)
library(vroom)
library(tidyquant)
library(rvest)    # scrape websites
library(purrr)  
library(lubridate) #to handle dates
library(ggrepel)
```

I'm a big fan watching movies. In this project, I will look at a subset sample of movies, taken from the Kaggle IMDB 5000 movie dataset. I would like to explore what factors influence the gross return of a movie. 


```{r,load_movies, warning=FALSE, message=FALSE}

movies <- read_csv(here::here("data", "movies.csv"))
glimpse(movies)

```

```{R,movies_by_genre}
#Produce a table with the count of movies by genre, ranked in descending order

movies %>% 
  group_by(genre) %>% 
  summarise(n=n()) %>% 
  arrange(desc(n))

```

First, I produce a table with the average gross earning and budget (`gross` and `budget`) by genre. Calculate a variable `return_on_budget` which shows how many $ did a movie make at the box office for each $ of its budget. Ranked genres by this `return_on_budget` in descending order. 

```{R,movies_average}
movies %>% 
  group_by(genre) %>% 
  summarise(avg_gross=mean(gross),
            avg_budget=mean(budget))

```
```{R,return_on_budget}
movies_ret <- movies %>% 
  group_by(genre) %>% 
  summarise(avg_gross=mean(gross),
            avg_budget=mean(budget)) %>% 
  mutate(return_on_budget=avg_gross/avg_budget) %>% 
  arrange(desc(return_on_budget))

movies_ret

```

Second, I produce a table that shows the top 15 directors who have created the highest gross revenue in the box office. 
```{R,movies_directors}
directors_top15 <- movies %>% 
  group_by(director) %>% 
  summarise(total_gross=sum(gross),
            mean_gross=mean(gross),
            median_gross=median(gross),
            stdev_gross=sd(gross)) %>% 
  arrange(desc(total_gross)) %>% 
  head(15)

directors_top15

```

-Lastly, I produce a table that describes how ratings are distributed by genre. 
```{R,movies_rating}

movies_rating_table<-movies %>% 
  group_by(genre) %>% 
  summarise(mean_rating=mean(rating),
            min_rating=min(rating),
            max_rating=max(rating),
            median_rating=median(rating),
            stdev_rating=STDEV(rating)) 

movies_rating_table

ggplot(
  data=movies_rating_table,
  mapping=aes(
    x=mean_rating))+
  geom_density()

ggplot(
  data=movies_rating_table,
  mapping=aes(
    x=min_rating))+
  geom_density()
 
ggplot(
  data=movies_rating_table,
  mapping=aes(
    x=max_rating))+
  geom_density()

ggplot(
  data=movies_rating_table,
  mapping=aes(
    x=median_rating))+
  geom_density()

ggplot(
  data=movies_rating_table,
  mapping=aes(
    x=stdev_rating))+
  geom_density()
```



To further visualize my findings, I will examine the relationship between the gross return of movies and their facebook likes.
  
```{r, gross_on_fblikes}
ggplot(
  data=movies,
  mapping=aes(
    x=cast_facebook_likes,
    y=gross))+
    geom_point()+
  scale_x_log10()+
  geom_smooth()

```
As illustrated by the plot, the number of facebook likes that the cast has received is not likely to be a good predictor of how much money a movie will make at the box office but it can be a good indicator when the number of likes reaches a certain level. 


Then, I will examine the relationship between the gross return of movies and and their budget. 

```{r, gross_on_budget}
ggplot(
  data=movies,
  mapping=aes(
    x=budget,
    y=gross))+
    geom_point()+
  geom_smooth()
```

I find that the budget is likely to be a good indicator of how much money a movie will make at the box office. 
  
  
I produce a similar plot for IMDB ratings. 

```{r, gross_on_rating}
gross_rating_scatterplot<-
  ggplot(
    data=movies,
    mapping=aes(
      x=rating,
      y=gross,
      color=genre))+
    geom_point()+
  scale_y_log10()

gross_rating_scatterplot+
  facet_wrap(~genre)
```

According to this plot, IMDB ratings are likely to be a good predictor of how much money a movie will make at the box office for certain genres such as action and comedy but it is difficult to make assumptions for some genres such as thriller, musical and romance due to lack of data.
 

